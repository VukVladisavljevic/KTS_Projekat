...
public class LineControllerIT {
    ...
    //-----------------get lines map------------------------------------
    @Test
    public void testLinesMap(){
        HttpEntity<String> entity = new HttpEntity<String>(null, headers);

        ParameterizedTypeReference<ArrayList<MapLinesDTO>> myBean =
                new ParameterizedTypeReference<ArrayList<MapLinesDTO>>() {};

        ResponseEntity<ArrayList<MapLinesDTO>> response = restTemplate.exchange(
                createURLWithPort("api//lines/map"),
