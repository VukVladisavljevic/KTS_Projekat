...
public class TimeScheduleController {
    ...
    public ResponseEntity<TimeSchedule> create(@RequestBody TimeSchedule schedule){
        TimeSchedule newSchedule = timeScheduleService.addNewSchedule(schedule);
    ...
    @RequestMapping(value = "/time-schedule/{scheduleId}", method = RequestMethod.PUT,
            consumes = MediaType.APPLICATION_JSON_VALUE,
            produces = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<TimeSchedule> update(@RequestBody TimeSchedule schedule, @PathVariable Long scheduleId){
        TimeSchedule updatedSchedule = timeScheduleService.update(scheduleId, schedule);
